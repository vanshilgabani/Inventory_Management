...
const numberToWords = (num) => {
  ...
  let words = [];
  if (crore > 0) words.push(convertTwoDigit(crore) + ' Crore');
  if (lakh > 0) words.push(convertTwoDigit(lakh) + ' Lakh');
  if (thousand > 0) words.push(convertTwoDigit(thousand) + ' Thousand');
  if (hundred > 0) words.push(ones[hundred] + ' Hundred');
  if (remainder > 0) words.push(convertTwoDigit(remainder));

  return words.join(' ').trim();
};

// Logo loading with multiple fallbacks
const getLogoPath = () => {
  const possiblePaths = [
...
const generateChallanPDF = async (order, challanSettings) => {
  return new Promise((resolve, reject) => {
    try {
      ...
      if (!order) return reject(new Error('Order data is required'));
      if (!order.items || order.items.length === 0) return reject(new Error('Order must contain items'));
      ...
      // Define Settings Data - **KEEP AS IS, NO CHANGES**
      ...
      const drawCell = (text, x, y, w, h, align = 'left', font = 'Helvetica', fontSize = 9, isBold = false) => {
        ...
        doc.font(isBold ? 'Helvetica-Bold' : font)
          .fontSize(fontSize)
          .text(String(text), x + 4, y + 5, { width: w - 8, align: align, lineBreak: true, ellipsis: true });
      };
      ...
      doc.font('Helvetica-Bold').fontSize(14)
        .text('CHALLAN', splitX, infoY, { width: rightBoxWidth, align: 'center' });
      ...
      for (let i = 0; i < range.count; i++) {
        ...
        doc.text(`Page ${i + 1} of ${totalPages}`, margin, pageHeight - 20, { width: contentWidth, align: 'right' });
      }
