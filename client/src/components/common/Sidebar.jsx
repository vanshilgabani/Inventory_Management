...
const Sidebar = ({ sidebarOpen, setSidebarOpen }) => {
  ...
  const [syncMode, setSyncMode] = useState(() => {
    return user?.syncPreference === 'manual' ? 'manual' : 'auto';
  });
  ...
  const [expandedSections, setExpandedSections] = useState({
    ...
    if (user?.syncPreference) {
      const mode = user.syncPreference === 'manual' ? 'manual' : 'auto';
      console.log('🎯 Setting sync mode from user.syncPreference:', user.syncPreference, '→', mode);
      setSyncMode(mode);
    }
  }, [user?.syncPreference]);
  ...
  const menuItems = [
    ...
    {
      key: 'factory-receiving',
      path: '/factory-receiving',
      icon: FiTruck,
    ...
    },
    {
      ...
      tenantOnly: true,
      // ✅ NEW: Dynamic badge based on sync mode
      dynamicBadge: true,
      getBadge: () => (syncMode === 'auto' ? 'Auto' : 'Manual'),
      getBadgeColor: () =>
        syncMode === 'auto' ? 'from-green-500 to-emerald-500' : 'from-orange-500 to-amber-500',
    },
    {
      key: 'wholesale',
      path: '/wholesale',
  ...
  return (
    <>
      ...
      {/* Sidebar Container */}
      <aside
        className={`fixed top-0 left-0 z-50 h-screen w-72 bg-white shadow-2xl transform transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:z-0 flex flex-col ${
              ...
              {visibleMenuItems.map((item, index) => {
                        ...
                        <div className="ml-8 mt-1 space-y-1 animate-fadeIn">
                          {item.submenu.map((subItem) => (
                            ...
                            >
                              ...
                              {subItem.badge && (
                                <span
                                  className={`px-2 py-0.5 text-xs font-semibold rounded-full bg-gradient-to-r ${subItem.badgeColor} text-white shadow-sm`}
                                >
                                  {subItem.badge}
                                </span>
                              )}
                ...
                return (
                  ...
                  >
                    ...
                    {/* ✅ Enhanced Badge Design - Dynamic or Static */}
                    {(item.badge || item.dynamicBadge) && (
                      <span
                        className={`
                          px-2.5 py-1 text-xs font-bold rounded-full 
                          bg-gradient-to-r ${item.dynamicBadge ? item.getBadgeColor() : item.badgeColor} 
                      ...
                      >
                        ...
                        {item.dynamicBadge ? item.getBadge() : item.badge}
                      </span>
                    )}
                  </NavLink>
                );
                  ...
                  {visibleSubscriptionItems.map((item, index) => {
                    ...
                    return (
                      <NavLink
                        ...
                        onClick={() => setSidebarOpen(false)}
                        className={({ isActive }) =>
                          `flex items-center justify-between px-4 py-2.5 rounded-lg transition-all duration-200 transform hover:scale-1.02 ${
                            isActive
                              ? 'bg-gradient-to-r from-blue-500 to-purple-600 text-white shadow-md'
                              : 'text-gray-700 hover:bg-gray-100'
                          }`
                      ...
                      >
                        ...
                        {item.badge && (
                          <span
                            className={`px-2 py-0.5 text-xs font-semibold rounded-full bg-gradient-to-r ${item.badgeColor} text-white shadow-sm`}
                          >
                            {item.badge}
                          </span>
                        )}
                  ...
                  {visibleAdminItems.map((item, index) => {
                    ...
                    return (
                      <NavLink
                        ...
                        onClick={() => setSidebarOpen(false)}
                        className={({ isActive }) =>
                          `flex items-center space-x-3 px-4 py-2.5 rounded-lg transition-all duration-200 transform hover:scale-1.02 ${
                            isActive
                              ? 'bg-gradient-to-r from-blue-500 to-purple-600 text-white shadow-md'
                              : 'text-gray-700 hover:bg-gray-100'
                          }`
